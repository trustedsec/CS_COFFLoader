...
namespace COFFLoader
{
    unsafe class CoffParser 
    {
        ...
        private static byte[] Subset( byte[] src, int offset, int size )
        {
            byte[] dst = new byte[size];
            System.Buffer.BlockCopy(src, offset, dst, 0, size);
            return dst; 
        }
        private static byte[] Subset( byte* src, int offset, int size )
        {
        ...
        private static int StringLength( byte[] coff_data, int offset )
        {
            int i = 0;
            while( coff_data[offset+i] != 0)
                i = i + 1; 
              
            return i;
        }
        private static int strlen(char* data)
        {
            int size = 0;
            while (data[size] != '\x00')
            {
        ...
        private static void* process_symbol(string symbolstring)
        {
            ...
            if (symbolstring.Contains("__ms_")) PREPENDSYMBOLVALUELEN = 5;
        ...
        public static int parseCOFF(byte[] functionname, byte[] data, int filesize, byte[] argumentdata, int argumentSize)
        {
            ...
            if (isBeaconObject == true)
            {
                ...
                beaconFunctionMapping = Win32.VirtualAlloc(IntPtr.Zero, 2048, (uint)(Win32.AllocationType.Commit|Win32.AllocationType.Reserve|Win32.AllocationType.TopDown), (uint)Win32.MemoryProtection.PAGE_EXECUTE_READWRITE);
                coffFunctionMapping = Win32.VirtualAlloc(IntPtr.Zero, 2048, (uint)(Win32.AllocationType.Commit|Win32.AllocationType.Reserve|Win32.AllocationType.TopDown), (uint)Win32.MemoryProtection.PAGE_EXECUTE_READWRITE);
            ...
            {
                Debug.WriteLine("header size: 0x{0:X} size char 0x{1:X} size byte 0x{2:X}", sizeof(COFF_FILE_HEADER), sizeof(char), sizeof(byte) );
                Debug.WriteLine("sect size: {0}, 0x{1:X} 0x{2:X}", counter, sizeof(COFF_SECT), counter*sizeof(COFF_SECT));
                coff_sect = (COFF_SECT*)(coff_data + sizeof(COFF_FILE_HEADER) + (counter*sizeof(COFF_SECT)));
...
//                if ( rawSize == 0) continue;
                ...
                Debug.WriteLine("size: 0x{0:X}\nalloctype: 0x{1:X}\nProtect: 0x{2:X}", rawSize, (uint)(Win32.AllocationType.Commit|Win32.AllocationType.Reserve|Win32.AllocationType.TopDown), (uint)Win32.MemoryProtection.PAGE_EXECUTE_READWRITE);
                IntPtr tmpAddr = Win32.VirtualAlloc(IntPtr.Zero, (uint)rawSize, (uint)(Win32.AllocationType.Commit|Win32.AllocationType.Reserve|Win32.AllocationType.TopDown), (uint)Win32.MemoryProtection.PAGE_EXECUTE_READWRITE);
            ...
            for (counter = 0; counter < coff_header->NumberOfSections; counter++)
            {
                ...
                for (reloccount = 0; reloccount < coff_sect->NumberOfRelocations; reloccount++)
                {
                    ...
                    coff_reloc = (COFF_RELOC*) (coff_data + coff_sect->PointerToRelocations + (sizeof(COFF_RELOC)*reloccount));
                    Debug.WriteLine(StructHelper.PrintStruct(coff_reloc));

                    coff_sym = (COFF_SYM*)( coff_data + coff_header->PointerToSymbolTable + (coff_reloc->SymbolTableIndex * sizeof(COFF_SYM)));
                    ...
                    if(coff_sym->value_u[0] != 0 )
                    {
                        if (coff_reloc->Type == Win32.IMAGE_REL_AMD64_ADDR64) // Type == 1 relocation is the 64-bit VA of the relocation target 
                        {
                            ...
                            longoffsetvalue = (ulong)Marshal.ReadInt64( new IntPtr(sectionMapping[counter].ToInt64() + coff_reloc->VirtualAddress ));
                            Debug.WriteLine("\tReadin longOffsetValue : 0x{0:llX}", longoffsetvalue);
                            longoffsetvalue = (ulong)sectionMapping[coff_sym->SectionNumber-1].ToInt64() + longoffsetvalue;
                            Debug.WriteLine("\tModified longOffsetValue : 0x{0:llX} Base Address: 0x{1:llX}", longoffsetvalue, sectionMapping[coff_sym->SectionNumber-1]);
                            Marshal.WriteInt64( new IntPtr(sectionMapping[counter].ToInt64() + coff_reloc->VirtualAddress), (long)longoffsetvalue );
                        } else if (coff_reloc->Type == Win32.IMAGE_REL_AMD64_ADDR32NB) /* This is Type == 3 relocation code */
                        {
                            ...
                            offsetvalue = (uint)Marshal.ReadInt32( new IntPtr(sectionMapping[counter].ToInt64() + coff_reloc->VirtualAddress ));
                            ...
                            Debug.WriteLine("\t\tSetting 0x{0:X} to 0x{1:X}\n", sectionMapping[counter].ToInt64()+coff_reloc->VirtualAddress, offsetvalue);
                    ...
                    }else
                    {
                        ...
                        int offset = coff_header->PointerToSymbolTable + (coff_header->NumberOfSymbols * sizeof(COFF_SYM)) + symptr;
                        string functionName =  Marshal.PtrToStringAnsi(new IntPtr((char*)(coff_data+offset)));
                        ...
                        {
                            Console.WriteLine("Failed to resolve symbol\n");
                            retcode = 1;
                            goto cleanup;
                        }
						if (coff_reloc->Type == Win32.IMAGE_REL_AMD64_REL32)
						{
                            ...
                            Debug.WriteLine("\tsectionMapping[{0}] \t 0x{1:X}", counter, sectionMapping[counter].ToInt64());
                            Debug.WriteLine("\tdifference 0x{0:X}", tmp - sectionMapping[counter].ToInt64());
                            ...
                            Win32.memcpy(new IntPtr(a), BitConverter.GetBytes(new IntPtr(funcptrlocation).ToInt64()), sizeof(long));
                            Debug.WriteLine("\tfunctionMapping addr:\t0x{0:X}", a);
                            Debug.WriteLine("\t\tRelative address : 0x{0:X}", new IntPtr(b-4).ToInt64());
                            Debug.WriteLine("\t\toffset value: 0x{0:X}", (a-b));
                            Win32.memcpy(new IntPtr(b-4), BitConverter.GetBytes(a-b), sizeof(uint));
                            functionMappingCount++;
						}
						else if (coff_reloc->Type >= Win32.IMAGE_REL_AMD64_REL32 && coff_reloc->Type <= Win32.IMAGE_REL_AMD64_REL32_5)
						{
                            Debug.WriteLine("coff_sym->value_u[0] == 0  <==> coff_reloc->type between 4 and 9");
                            ...
                            offsetvalue1 += sectionMapping[coff_sym->SectionNumber-1].ToInt64() - b;
            ...
            {
                coff_sym = (COFF_SYM*)( coff_data + coff_header->PointerToSymbolTable + (tempcounter * sizeof(COFF_SYM)));
                Debug.WriteLine(StructHelper.PrintStruct(coff_sym));
                if( isBeaconObject == false)
                {
                    if (memcmp( coff_sym->Name, Encoding.Default.GetString(functionname)) == 0 )
                    {
                        Debug.WriteLine("\t\tFound entry {1}! \n\t\t\t Address to execute: 0x{0:X}", sectionMapping[coff_sym->SectionNumber-1].ToInt64()+coff_sym->Value, Encoding.Default.GetString(functionname));
                        tmpFuncDelegate foo = (tmpFuncDelegate)Marshal.GetDelegateForFunctionPointer(new IntPtr(sectionMapping[coff_sym->SectionNumber-1].ToInt64()+coff_sym->Value), typeof(tmpFuncDelegate) );
                        int size = Encoding.Default.GetString(argumentdata).Length;
                        byte* funcName = (byte*)Win32.VirtualAlloc(IntPtr.Zero, (uint)size, (uint)(Win32.AllocationType.Commit|Win32.AllocationType.Reserve|Win32.AllocationType.TopDown), (uint)Win32.MemoryProtection.PAGE_EXECUTE_READWRITE);
                        ...
                        Win32.VirtualFreeEx( IntPtr.Zero, (IntPtr)funcName, IntPtr.Zero, Win32.AllocationType.Release );
                        break;
                    }
                    
                }else
                {
                    if (coff_sym->value_u[0] != 0 || coff_sym->Type != 0x20 || coff_sym->SectionNumber != 1) continue;
                    string functionName =  Marshal.PtrToStringAnsi(new IntPtr((char*)(coff_data+ coff_header->PointerToSymbolTable + (coff_header->NumberOfSymbols * sizeof(COFF_SYM)) + coff_sym->value_u[1])));
                    Debug.WriteLine("\tFunction Name {0} ", functionName);
                    string localfunc;

                    localfunc = functionName;
                    Debug.WriteLine("\tFunction Name {0} Hash: 0x{1:X}", localfunc, hash_djb( Encoding.Default.GetBytes(localfunc)));
                    IntPtr functionAddress = new IntPtr(sectionMapping[coff_sym->SectionNumber-1].ToInt64()+coff_sym->Value);
                    BeaconInternalMapping.Add( new BEACON_FUNCTION(hash_djb( Encoding.Default.GetBytes(localfunc)), (void*)functionAddress.ToPointer()));
                }
            }
            cleanup:
            if( isBeaconObject == false)
            {
                ...
                if ( beaconFunctionMapping != IntPtr.Zero)
                    Win32.VirtualFreeEx( IntPtr.Zero, beaconFunctionMapping, IntPtr.Zero, Win32.AllocationType.Release );
                if ( coffFunctionMapping != IntPtr.Zero)
                    Win32.VirtualFreeEx( IntPtr.Zero, coffFunctionMapping, IntPtr.Zero, Win32.AllocationType.Release );
            }
            return retcode;
        }
